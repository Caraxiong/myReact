...
//    （1）、拿新的子节点树跟以前老的子节点树对比，找出他们之间的差别。我们称之为diff
//    （2）、所有差别找出后，再一次性的去更新。我们称之为patch
...
ReactDOMComponent.prototype._updateDOMProperties = function(lastProps, nextProps) {
    ...
    for (propKey in lastProps) {
        ...
        // 这样剩下的都是不在新属性集合里的。需要删除，后边执行$('[data-reactid="' + this._rootNodeID + '"]').removeAttr(propKey)
...
var diffQueue = [];
//更新子节点包含两个部分，
// 1、一个是递归的分析差异，把差异添加到队列中。
// 2、在合适的时机调用_patch把差异应用到dom上。
...
// _diff内部也会递归调用子节点的receiveComponent于是当某个子节点也是浏览器普通节点，就也会走_updateDOMChildren这一步。
// 所以这里使用了updateDepth来记录递归的过程，只有等递归回来updateDepth为0时，代表整个差异已经分析完毕，可以开始使用patch来处理差异队列了

/** _diff start */
/**
    整体思路：
    1、首先我们拿到之前的component的集合，如果是第一次更新的话，这个值是我们在渲染时赋值的。然后我们调用generateComponentChildren生成最新的component集合。
    我们知道component是用来放element的，一个萝卜一个坑。

    2、注意flattenChildren我们这里把数组集合转成了对象map,以element的key作为标识，当然对于text文本或者没有传入key的element,直接用index作为标识。通过这些标识，
    我们可以从类型的角度来判断两个component是否是一样的。

    3、generateComponentChildren会尽量的复用以前的component，也就是那些坑，当发现可以复用component（也就是key一致）时，就还用以前的，只需要调用他对应的更新方法
    receiveComponent就行了，这样就会递归的去获取子节点的差异对象然后放到队列了。如果发现不能复用那就是新的节点，我们就需要instantiateReactComponent重新生成一个
    新的component。
*/
// 差异更新的几种类型
var UPATE_TYPES = {
...
/** patch start */
// _patch主要就是挨个遍历差异队列，遍历两次，第一次删除掉所有需要变动的节点，然后第二次插入新的节点还有修改的节点。
//用于将childNode插入到指定位置
...
//更新
// 首先会合并改动，生成最新的state,props然后拿以前的render返回的element跟现在最新调用render生成的element进行对比（_shouldUpdateReactComponent），
